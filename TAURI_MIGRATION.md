...
- **复验方法**：仓库根 `npx tauri build --no-bundle`（exe 被旧进程占用时先 `taskkill //IM app.exe //F`）→ `DESK_DEBUG_PORT=9224 ./app.exe` 启动（调试端口默认关闭，H4 修复后必须显式设 env 才有 CDP）→ CDP 9224 断言（任意 CDP 客户端均可；联调靶子 `node scripts/fake-llm.js 8788`，BaseURL 填 http://127.0.0.1:8788/v1，model 随意，127.0.0.1 免 Key）；Electron 回归 = `DESK_DEBUG_PORT=9223 npm run dev` + CDP 9223（同机勿双版本同时跑，共享 DB 有并发写风险）；Node 侧 `npm test` 必须全绿。打包产物同样支持 DESK_DEBUG_PORT（env 是 main 进程启动期读取的，与 dev/打包无关）。
- **开机自启 + 双壳四形态打包（2026-09-25）**：①开机自启——官方 tauri-plugin-autostart（capabilities 放行三命令，desk 直 invoke 插件通道、不加 npm 包），`settings.autoStart` 存意图 + 两壳启动对账自愈；Electron 登录项名显式钉死「摸鱼桌宠」——`getLoginItemSettings` 无 name 参数且 openAtLogin 的 path+args 比对不可靠（实测恒 false），改按 `launchItems[].name` 匹配；auto-launch 0.5 对缺失条目 is_enabled/disable 抛 os error 2（非幂等），两方向归一化。②打包——build.js ignore 收紧（12G→381M：src-tauri/target 曾整体进 asar，内部文档随包外流）；productName 改中文「摸鱼桌宠」；WebView2 UDF 显式指 `%LOCALAPPDATA%\com.yuki.deskpet\WebView2`（windows.rs 建窗统一装饰点，零安装钩子——模板已内置 Run 条目清理，StartupApproved 伴生值残留不可见、接受）；Electron NSIS 脚本 scripts/installer.nsi（per-user/HKCU/固定目录免 UAC/自启条目指向核对才删/无数据删除选项）。方案与红线全录：AUTOSTART_PLAN.md、PACKAGING_PLAN.md。
