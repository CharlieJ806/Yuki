...
export function createService(store, deps = {}) {
  ...
  async function clearGallery(sessionId) {
    ...
    /* 直接从键名表生成，加类目时不用再改这里（含 photo 类目） */
    for (const k of Object.values(GALLERY_KEYS)) {
      await store.setMeta(k.list, null)
      await store.setMeta(k.mem, null)
    }
  }
  ...
  async function appendUnlockPhoto(sessionId, unlocked) {
    try {
      ...
      for (const part of parts) {
        const msg = await store.addMessage(sessionId, 'assistant', part.content, {
